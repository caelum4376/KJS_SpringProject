...
public class DataBoardController {
	...
	public String databoard_insert_ok(DataBoardVO vo) {
		...
		if (list == null) { // 파일이 없는 상태 (업로드가 안된 경우)
			...
			for (MultipartFile mf:list) {
				File file = new File("/Users/caelum/download/"+mf.getOriginalFilename());
				try {
	...
	// 요청데이터가 없는 경우 : String으로 설정
	// 모든 데이터는 String으로 받을 수 있다
	// 매개변수는 순서가 없다, 여러개를 받을 수 있다
	/*
	 * 		MVC의 핵심 => 매개변수 / 리턴형
	 * 								 ------ 화면이동 (forward / sendRedirect)
	 * 										forward => 화면변경 => request
	 * 											return "main/main"
	 * 										sendRedirect => 화면변경 => request를 초기화
	 * 													 => 기존의 설정된 위치로 변경
	 * 											return "redirect:.do"
	 ...
	 * 				---------------------------------------
	 * 					=> 다운로드, Cookie사용
	 * 				HttpSession, Model, RedirectAttribute
	 * 				PasswordEncoder
	 * 		DAO : 데이터베이스 연동
	 * 		Controller : 조립기 (DAO연동 => 데이터를 추출 => 브라우저 전송)
	 * 		VO : 관련된 데이터를 모아서 관리 (사용자 정의 데이터형)
	 * 		Manager : OpenAPI
	 * 		RestConroller : JavaScript로 JSON을 전송할 목적
	 * 		------------- 다른 언어와 연결 => 데이터를 전송
	 * 					  ---------
	 * 					1. WEB => JavaScript(Ajax, VueJS, React)
	 * 					2. Mobile => Kotlin
	 * 		Service : BI => DAO여러개를 통합, 다른 서비스 통합 (크롤링) => 의존성이 낮은 프로그램
	 */
	...
	public void databoardDownload(String fn, HttpServletResponse response) {
		try {
			File file = new File("/Users/caelum/download/"+fn);
