 ...
 * 											POJO (독립적인 클래스 제작)
 * 					2) 메소드마다 반복해서 사용하는 소스코드 => 공통모듈로 분리 (응집력이 높은 프로그램 제작)
 * 						=> AOP : OOP를 보완
 * 				=> 클래스를 모아서 관리 (생성 ~ 소멸) => 컨테이너
 * 					생성에 필요한 데이터나 객체 주소가 필요할때도 있다 : DI
 * 					
 * 					*** DI (객체 생성, 클래스의 연관관계)
 * 					*** AOP : 공통 모듈 (AOP기반 => 로그파일, 트랜젝션, 보안)
 * 					*** DI/AOP => 인터페이스 기반
 * 		2. 컨테이너의 종류
 * 			BeanFatory : 가장 단순한 컨테이너
 * 				|		 사용자가 만든 클래스만 관리 => DI
 * 				|		 --------------------------------- core
 * 			ApplicationContext(Interface) : AOP, 메세지 지원 (JMS)
 * 				|				=> 구현된 클래스
 * 				|				   ClassPathApplicationContext(Class)
 ...
 * 					@Controller : MVC구조의 Model (페이지 저장)
...
public class MainClass {
	public static void main(String[] args) {
		String path = "/Users/caelum/Documents/springDev/SpringStudy/SpringDIProject08/src/main/java/com/sist/main/app.xml";
