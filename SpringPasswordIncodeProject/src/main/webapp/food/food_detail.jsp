...
<script type="text/javascript" src="https://dapi.kakao.com/v2/maps/sdk.js?appkey=d3be6802e7c6f6cf640550fd67e44d41&libraries=services"></script>
			...
			<div class="col-sm-7">
				...
				<table class="table">
					...
					<tr v-if="food_detail.menu!='no'">
						...
						<td width="80%">
							<ul>
								<li v-for="m in menu" v-if="m!=''">{{m}}원</li>
							</ul>
						</td>
					</tr>
				</table>
					...
					<tr>
						<td>
							<table class="table" v-for="re in reply_data">
								<tr>
									<td class="text-left">
										☺︎<span style="color:blue;">{{re.name}}</span>&nbsp;{{re.dbday}}
									</td>
									<td class="text-right">
										<span v-if="re.id == food_detail.sessionId">
											...
											<input type="button" class="btn btn-xs btn-danger" value="삭제" @click="replyDelete(re.no)">
										</span>
									</td>
								</tr>
								<tr>
								...
								<tr style="display:none;" :id="'reply'+re.no" class="updates">
									<td colspan="2">
										<textarea rows="3" cols="55" style="float: left" id="uMsg">{{re.msg}}</textarea>
										<input type="button" value="댓글수정" style="height:69px; 
				...
				<table class="table" v-if="food_detail.sessionId != null">
					<tr>
						<td>
							...
							<input type="button" value="댓글쓰기" style="height:69px; 
		...
		new Vue({
			...
			methods:{
				...
				addScript:function(){
					...
					script.src = 'https://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=d3be6802e7c6f6cf640550fd67e44d41&libraries=services'
				...
				replyUpdate:function(no){
					...
					axios.post('../food/reply_update_vue.do',null,{
						...
						console.log(response.data)
						this.reply_data = response.data // updated => 변경된 데이터값을
						this.msg = ''
						$('#reply'+no).hide()
						$('#up'+no).val("수정")
						this.no=0
